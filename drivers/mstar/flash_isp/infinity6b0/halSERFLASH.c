...
extern void flush_cache(MS_U32 start, MS_U32 size);
...
#endif
		ApkShowProgress((int)((u32I-u32StartBlock+1)*100/(u32EndBlock-u32StartBlock+1)));
...
#endif
		ApkShowProgress((int)((u32TotalSize-u32Size)*100/(u32TotalSize))); 
    }
...
/*
        ...
        printk("\b\b\b\b%3d%%", (int)((u32TotalSize-u32Size)*100/(u32TotalSize)) );
#endif
		ApkShowProgress((int)((u32TotalSize-u32Size)*100/(u32TotalSize)));2222
    }
...
MS_BOOL HAL_FSP_BlockErase(MS_U32 u32StartBlock, MS_U32 u32EndBlock, MS_BOOL bWait)
{
    ...
    for( u32Idx = u32StartBlock; u32Idx <= u32EndBlock; u32Idx++ )
    {
        ...
        if( ((get_timer(0)-u32start)>500) ||  (u32Idx==u32EndBlock))
        {
			...
			ApkShowProgress((int)((u32Idx-u32StartBlock+1)*100/(u32EndBlock-u32StartBlock+1)));
            u32start = get_timer(0);
        }
#endif
		WATCHDOG_RESET();
...
MS_BOOL HAL_FSP_WriteProtect_Area(MS_BOOL bEnableAllArea, MS_U8 u8BlockProtectBits)
{
    ...
    return bRet;
#else
	//printf("HAL_FSP_WriteProtect_Area :0x%x=0x%x\n", bEnableAllArea, u8BlockProtectBits);
	return TRUE;
#endif
}
...
MS_BOOL HAL_FSP_WriteProtect(MS_BOOL bEnable)
{
    ...
    return bRet;
#else
	//printf("HAL_FSP_WriteProtect :bEnable=%d\n", bEnable);
	return TRUE;
#endif
}
...
static MS_BOOL _HAL_PP_BDMAToFSP(MS_U32 u32Src_off, MS_U32 u32Dst_off, MS_U32 u32Len)
{
    ...
    // Set start address
    WRITE_WORD((_hal_isp.u32BdmaBaseAddr + (0x4<<2)), (u32Src_off & 0x0000FFFF));
    WRITE_WORD((_hal_isp.u32BdmaBaseAddr + (0x5<<2)), (u32Src_off >> 16)&0x0FFF);
    // Set end address
...
int APK_SPI_Flash_Cmd(unsigned char cmd, int bRead, char* pData, int iSize)
{
	int i = 0;
	MS_BOOL bRet = TRUE;
...
hal_SERFLASH_t* Apk_Get_SpiFlash_info()
{
	return &_hal_SERFLASH;
}
