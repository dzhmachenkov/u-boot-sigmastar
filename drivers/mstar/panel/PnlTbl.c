...
void Apk_spi_reset(void)
{
	_toggle_SPI_RST(0);
	Delay_ms(100);
	_toggle_SPI_RST(1);
}
int _SPI_Readbits(u32 value,int bits)
{
	unsigned int i;
	unsigned int MSB_First = 1<<(9-1);
	...
	for(i=0;i< bits ;i++)
	{
		...
		if(i == 0)
		{
			MDrv_GPIO_Pad_Set(u32GPIOSDI);
			mdrv_gpio_set_input(u32GPIOSDI);
		}else{
			tmpValue |= mdrv_gpio_get_level(u32GPIOSDI);
			tmpValue = tmpValue<<1;
		}
		Delay_us(1);
		_toggle_SPI_SCL(1);
...
void Lcm_init_Ini(unsigned short *stLCMTable, u32 u32CmdCount,struct spi_attr stSpiCfg)
{
    ...
    u16 n = 0;
	LCM_setting_table mPnlInitCmdTable[800];
    ...
    for (n = 0; n < u16PanelCmdCnt; n++)
    {
        if (REGFLAG_DELAY == mPnlInitCmdTable[n].cmd)
        {
            ...
            Delay_ms(mPnlInitCmdTable[n].count); //lzj
