...
MDrv_SPINAND_Init_Detect_ID:
    ...
    if(!_MDrv_SPINAND_GET_INFO())
    {
        ...
        tSpinandInfo->au8_ID[2] = u8ID[2];
        tSpinandInfo->u8_IDByteCnt = 3;
        return FALSE;
#if 0
...
U32 MDrv_SPINAND_ReadData(U32 u32_PageIdx, U8 *u8Data, U8 *pu8_SpareBuf, U32 u32_DataSize, void *lfsPartitionInfo)
{
    U8 u8Status;
    ...
    U16 u16BlkCnt = 0;
    U16 u16BlkPageCnt = 0;
    FS_PARTITION_Info_t *lfsPartInfo = (FS_PARTITION_Info_t*)lfsPartitionInfo;
    if(lfsPartInfo->u8_UseExternBlockSize)
    {
        u16BlkCnt = lfsPartInfo->u16_BlkCnt;
        u16BlkPageCnt = lfsPartInfo->u32_BlkSize / lfsPartInfo->u32_PageSize;
    }
    else
    {
        u16BlkCnt = pSpiNandDrv->tSpinandInfo.u16_BlkCnt;
        u16BlkPageCnt = pSpiNandDrv->tSpinandInfo.u16_BlkPageCnt;
    }
    if( ((pSpiNandDrv->tSpinandInfo.au8_ID[0] == 0xEF) &&
         (pSpiNandDrv->tSpinandInfo.au8_ID[1] == 0xAB) &&
         (pSpiNandDrv->tSpinandInfo.au8_ID[2] == 0x21)) ||
        ((pSpiNandDrv->tSpinandInfo.au8_ID[0] == 0xC8) &&
         (pSpiNandDrv->tSpinandInfo.au8_ID[1] == 0x0A)) )
    {
        U16 u16DiePageCnt = MDrv_SPINAND_CountBits(u16BlkCnt * u16BlkPageCnt);
    ...
    // Read SPINand Data
    if(pSpiNandDrv->tSpinandInfo.u8PlaneCnt && (((u32_PageIdx/u16BlkPageCnt)&0x1) == 1))
    {
...
#endif
    u32Ret = HAL_SPINAND_Read (u16ColumnAddr,u32_DataSize, u8Data);
#if defined(SUPPORT_SPINAND_QUAD) && SUPPORT_SPINAND_QUAD
...
void MDrv_SPINAND_GetPartitionInfo(char *partition, void *pPartInfo)
{
    u8 pnum;
    int ret;
    ...
    u8 u8BlkByteCntBits;
    u64 u64StartAddr,u64PartSize;
    FS_PARTITION_Info_t *pNandPartInfo  = (FS_PARTITION_Info_t *)pPartInfo;
    SPI_NAND_DRIVER_t *pSpiNandDrv = (SPI_NAND_DRIVER_t*)drvSPINAND_get_DrvContext_address();
    ...
    if (ret)
    {
        printf("find %s part fail!\n", partition);
        return;
    }

    u64StartAddr = part->offset;
    u64PartSize = part->size;
    if(pNandPartInfo->u8_UseExternBlockSize)
    {
        u8BlkByteCntBits = MDrv_SPINAND_CountBits(pNandPartInfo->u32_BlkSize);
    }
    else
    {
        pNandPartInfo->u32_BlkSize = part->sector_size;
        u8BlkByteCntBits = MDrv_SPINAND_CountBits(part->sector_size);
    }
    pNandPartInfo->u16_StartBlk = (u64StartAddr >> u8BlkByteCntBits);
    pNandPartInfo->u16_BlkCnt = (u64PartSize >> u8BlkByteCntBits);
    pNandPartInfo->u32_PageSize = pSpiNandDrv->tSpinandInfo.u16_PageByteCnt;
}


U16 DrvSpinand_FindPartTypeByName(const char* partName)
{
    if(strcmp(partName, "MISC") == 0 )
        return UNFD_PART_MISC;
    else if(strcmp(partName, "IPL_CUST") == 0 )
        return UNFD_PART_IPL_CUST;
    else if(strcmp(partName, "KERNEL") ==0 )
        return UNFD_PART_KERNEL;
    else if((strcmp(partName, "IPL0") == 0 ) || (strcmp(partName, "IPL1") == 0 ))
    ...
    else if((strcmp(partName, "ENV0") == 0 ) || (strcmp(partName, "ENV1") == 0 ))
