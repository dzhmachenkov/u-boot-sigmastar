...
void  drawRect_Yuv_black(int x0, int y0, int width, int height, int stride, int iscrH, unsigned char* frameBuffer)
{
    const int bytesPerPixel =1;
 
    const char color8 = 0;
    char *dest = NULL;
	char* pDst420spUV=NULL;
    int x, y;
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            dest[x] = color8;
			pDst420spUV = frameBuffer+width*iscrH+y/2+x;
			*pDst420spUV = 0x80;
        }
        dest = (char *)((char *)dest + stride);
    }
}
static u32 g_ApkFrameBufferAddr=0;
static u32 g_ApkDispWidth=0;
static u32 g_ApkDispHeight=0;
u32 ApkDispGetFrameBufferAddr()
{
	return g_ApkFrameBufferAddr;
}
void ApkDispGetInfo(u32* pW, u32* pH)
{
	if(pW) *pW = g_ApkDispWidth;
	if(pH) *pH = g_ApkDispHeight;
}
...
static void _BootJpdYuvCtrl(u32 u32InBufSzie, u32 u32InBuf, u32 u32OutBufSize, u32 u32OutBuf, u16 *pu16OutWidth, u16 *pu16OutHeight, LogoRotation_e eRot)
{
    ...
    {
        BOOTLOGO_DBG(BOOTLOGO_DBG_LEVEL_ERR,"%s %d:: Output buffer is too big, %d>%d\n",
...
static int _MiscPartiton(u32 u32LogoId, DispOutAspectRatio *pstAspectRatio, LogoRotation_e eRot)
{
    ...
    INI_GET_INT(stDispOut.enDev, "LOGO:m_eDeviceType");
    _GetLogoReservedAddr(&u32Addr, &u32Size);
    stDispOut.phyAddr = (u64)u32Addr;
    stDispOut.u32Size = u32Size;
    
    INI_GET_INT(stDispOut.u32Width, "LOGO:m_wDispWidth");
    INI_GET_INT(stDispOut.u32Height, "LOGO:m_wDispHeight");
    INI_GET_INT(stDispOut.u32Clock, "LOGO:m_wDispFps");
	printf("DISP TYPE:%d\r\n", stDispOut.enDev);
    switch (stDispOut.enDev)
    {
	    ...
	    //_SPI_Init(17, 18, 19, 21);
	    //printf("_SPI_Readbits:%x\r\n", _SPI_Readbits(0x04,25));
	    if(DispID == 0x12a08)
	    {
	    	INI_GET_STR(pTargetPnlName, "LOGO:m_sParaTarget2");
	    }else 
	    {
	    	INI_GET_STR(pTargetPnlName, "LOGO:m_sParaTarget");
	    }
    ...
    //printf("%s:addr=%x,size=%d\n",stDispOut.phyAddr,stDispOut.u32Size);
    //printf("%s:u32Width=%x,u32Height=%d\n",stDispOut.u32Width,stDispOut.u32Height);
    
    char iniDst[32];
    sprintf(iniDst, "LOGO:m_sLogoFile%d", u32LogoId);
    INI_GET_STR(pFileName, iniDst);
...
#endif
#if defined(CONFIG_SSTAR_RGN)
    _BootJpdArgbCtrl(u32FileSize, (u32)inBuf, stDispOut.u32Size, (u32)stDispOut.phyAddr, (u16 *)&stDispOut.u32ImgWidth, (u16 *)&stDispOut.u32ImgHeight, eRot);
#else
	g_pdw_v_logo_Buff = stDispOut.phyAddr + 20<<20;	
	if(g_pdw_v_logo_Buff) //ld@apical	
	{		
		_BootJpdYuvCtrl(u32FileSize, (u32)inBuf, stDispOut.u32Size, (u32)g_pdw_v_logo_Buff, (u16 *)&stDispOut.u32ImgWidth, (u16 *)&stDispOut.u32ImgHeight, eRot);		
		memcpy( (u32)stDispOut.phyAddr+BOOTLOGO_VIRTUAL_ADDRESS_OFFSET, g_pdw_v_logo_Buff+BOOTLOGO_VIRTUAL_ADDRESS_OFFSET, stDispOut.u32ImgWidth*stDispOut.u32ImgHeight*3/2);	
	}
	else
	    _BootJpdYuvCtrl(u32FileSize, (u32)inBuf, stDispOut.u32Size, (u32)stDispOut.phyAddr, (u16 *)&stDispOut.u32ImgWidth, (u16 *)&stDispOut.u32ImgHeight, eRot);
#endif
    flush_dcache_all();
    free(inBuf);

	//lidan
	g_ApkFrameBufferAddr = u32Addr + BOOTLOGO_VIRTUAL_ADDRESS_OFFSET;
	g_ApkDispWidth = stDispOut.u32ImgWidth;
    g_ApkDispHeight = stDispOut.u32ImgHeight;
	extern void YUV420spRotatePic90(u8 *src, int w, int h);
	extern void ApkDrawInit();
	//ApkDrawInit();
	//YUV420spRotatePic90((u8*)g_ApkFrameBufferAddr, u16ImgWidth, u16ImgHeight);	
...
static int _LogoPartiton(u32 u32LogoId, DispOutAspectRatio *pstAspectRatio, LogoRotation_e eRot)
{
    ...
    if (idx == pHeader->u32DataInfoCnt || puDispPnl == NULL)
    {
		...
		//ld@apical
		if(u32LogoId == 0)
			ShowProgressBar("booting....", 300);
		else if(u32LogoId == 1)
			ShowProgressBar("updating....", 300);
		else if(u32LogoId==2)
			ShowProgressBar("update successed!", 300);
		else if(u32LogoId==3)
			ShowProgressBar("update failed!", 300);
		else if(u32LogoId==4)
			ShowProgressBar("Are you sure to update?", 300);
		else if(u32LogoId==5)
			ShowProgressBar("Please insert DC!", 300);
		//end
    ...
    free(pRawData);
	//lidan
	g_ApkFrameBufferAddr = pDispInfo->u32DispBufStart + BOOTLOGO_VIRTUAL_ADDRESS_OFFSET;
	g_ApkDispWidth = stDispOut.u32ImgWidth;
    g_ApkDispHeight = stDispOut.u32ImgHeight;
	extern void YUV420spRotatePic90(u8 *src, int w, int h);
	extern void ApkDrawInit();
...
int do_display (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	#if APICAL_LCD_LESS
	printf("bootolgo, but lcd less!\n");
	return 0;
	#endif
	
    u32 u32LogoId = 0;
	if(g_pdw_v_logo_Buff == NULL)
		g_pdw_v_logo_Buff = malloc(900*512*3/2);
...
#if defined(CONFIG_SSTAR_INI_PARSER)
	ret = _MiscPartiton(u32LogoId, &stAspectRatio, enRot);
	if(DispID == 0x12a08)
	{
		_MiscPartiton(u32LogoId, &stAspectRatio, enRot);
	}
	if (ret == -1)
		ret = _LogoPartiton(u32LogoId, &stAspectRatio, enRot);
#else
    ret = _LogoPartiton(u32LogoId, &stAspectRatio, enRot);
#endif
    bootstage_mark_name(BOOTSTAGE_ID_ALLOC, "do_display--");
...
int ApkLfsWriteBuffer(char* partName, char* fileName, int mode, char* buffer, int len, int offset)
{
    void *fd = NULL;
    int ret;
	...
	ret = inifs_mount(partName, NULL);
    if (ret)
    {
        fprintf(stderr, "inifs_mount %s return error %d\n", partName, ret);
        return ret;
    }

    fd = inifs_open(fileName, mode, 0);
    if (fd != NULL)
    {
        if (offset > 0)
            inifs_lseek(fd, offset, SEEK_SET);
        ret = inifs_write(fd, buffer, len);
        printf("write %s/%s %d bytes return %d\n",partName, fileName, len, ret);
    } else
    {
        fprintf(stderr, "inifs_open return NULL\n");
        inifs_unmount();
...
int ApkLfsReadToBuffer(char* partName, char* fileName, char* buffer, int len, int offset)
{
    void *fd = NULL;
    int ret;
